...
def _stage6_build_grid_batched(
    *,
    ...
    ifg_batch = _env_int("PYSTAMPS_STAGE6_GRID_IFG_BATCH", 4)
    window_batch = _env_int("PYSTAMPS_STAGE6_GRID_WINDOW_BATCH", 32)
    fft_workers = _env_int(
        "PYSTAMPS_STAGE6_GRID_FFT_WORKERS",
        max(1, min(16, os.cpu_count() or 1)),
    )
    resume = _env_bool("PYSTAMPS_STAGE6_GRID_RESUME", True)

    available = _stage6_grid_available_memory_bytes()
    if available > 0:
        per_ifg_bytes = int(n_i) * int(n_j) * 8 * 3
        safe_batch = max(1, int((available * 0.35) // max(1, per_ifg_bytes)))
        ifg_batch = min(ifg_batch, safe_batch)
    ifg_batch = max(1, min(ifg_batch, n_ifg))
    ...
    completed_initial = int(np.count_nonzero(done))
    ...
    metadata = {
        ...
        "active_windows": int(active_indices.size),
        "ifg_batch": int(ifg_batch),
        "window_batch": int(window_batch),
        "fft_workers": int(fft_workers),
        "completed": int(completed_initial),
    ...
    print(
        "[STAGE6_SBAS][GRID_V2] "
        f"windows={windows.shape[0]}, active={active_indices.size}, "
        f"ifg_batch={ifg_batch}, window_batch={window_batch}, "
        f"fft_workers={fft_workers}, resumed={completed_initial}/{n_ifg}",
        flush=True,
    )

    for ifg_start in range(0, n_ifg, ifg_batch):
        ifg_stop = min(ifg_start + ifg_batch, n_ifg)
        if np.all(done[ifg_start:ifg_stop] != 0):
            continue

        values = np.asarray(
            source[order, ifg_start:ifg_stop],
            dtype=np.complex64,
        )
        grouped = np.add.reduceat(values, starts, axis=0)

        current_batch = ifg_stop - ifg_start
        grid_stack = np.zeros(
            (int(n_i), int(n_j), current_batch),
            dtype=np.complex64,
            order="F",
        )
        ...
        flat_stack[group_lin, :] = grouped

        filtered, lowpass = _stage6_goldstein_filter_dense_batch(
            grid_stack,
            n_win=int(n_win),
            alpha=float(alpha),
        ...
        selected_filtered = filtered.reshape(
            (int(n_i) * int(n_j), current_batch),
            order="F",
        )[nz_flat, :]
        selected_lowpass = lowpass.reshape(
            (int(n_i) * int(n_j), current_batch),
            order="F",
        )[nz_flat, :]

        grid_phase[:, ifg_start:ifg_stop] = selected_filtered
        grid_lowpass[:, ifg_start:ifg_stop] = selected_lowpass
        grid_phase.flush()
        grid_lowpass.flush()

        done[ifg_start:ifg_stop] = 1
        done.flush()

        completed = int(np.count_nonzero(done))
        elapsed = time.perf_counter() - grid_started
        newly_completed = max(1, completed - completed_initial)
        rate = newly_completed / elapsed if elapsed > 0 else 0.0
        eta = (n_ifg - completed) / rate if rate > 0 else float("nan")
        ...
        del values, grouped, grid_stack, flat_stack
        del filtered, lowpass
        del selected_filtered, selected_lowpass
