...
def main():
    ...
    phase_marker = root / "_pystamps_stage8_phase_input.json"

    if phase_marker.is_file():
        try:
            marker_payload = json.loads(
                phase_marker.read_text(encoding="utf-8")
            )
            marker_phase = str(
                marker_payload.get("phase_file", "")
            ).strip()
            if marker_phase:
                phase_file = marker_phase
        except Exception as exc:
            raise PostprocessError(
                f"Unable to read Stage-8 phase provenance: {exc}"
            ) from exc

    phase_path = root / phase_file

    if not phase_path.is_file():
        raise PostprocessError(
            f"Stage-8 phase product does not exist: {phase_path}"
        )

    print("postprocess phase product :", phase_file)
