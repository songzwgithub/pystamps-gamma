...
def extract_phase_stack(
    ...
    *,
    ...
    if project.width is None or project.length is None:
        raise GammaInputError("工程尚未解析出干涉图行列数")

    row_array = np.asarray(rows, dtype=np.int64).reshape(-1)
    col_array = np.asarray(cols, dtype=np.int64).reshape(-1)

    if row_array.shape != col_array.shape:
        raise GammaInputError("rows与cols长度不一致")

    if max_invalid_interferograms < 0:
        raise GammaInputError("max_invalid_interferograms不能小于0")
    ...
    workers = max(
        1,
        min(
            int(os.environ.get("PYSTAMPS_STAGE1_IFG_WORKERS", "8")),
            n_ifg,
        ),
    )
    ...
    progress_seconds = max(
        0.5,
        float(os.environ.get("PYSTAMPS_STAGE1_PROGRESS_SECONDS", "3")),
    )

    print(
        f"[phase] 候选点={n_candidates:,}, IFG={n_ifg}, workers={workers}",
        flush=True,
    )
    ...
    def read_one(ifg_index: int) -> tuple[int, np.ndarray]:
        interferogram = project.interferograms[ifg_index]
        ...
        unit_phase = normalize_complex_unit(values)

        return ifg_index, unit_phase

    with ThreadPoolExecutor(
        max_workers=workers,
        thread_name_prefix="stage1-phase",
    ) as executor:
        futures = [
            executor.submit(read_one, ifg_index)
            for ifg_index in range(n_ifg)
        ]

        completed = 0

        for future in futures:
            ifg_index, unit_phase = future.result()
            ...
            if (
                now - last_report >= progress_seconds
                or completed == n_ifg
            ):
                elapsed = now - started
                ...
                print(
                    f"[phase] {completed}/{n_ifg} "
                    f"({completed/n_ifg*100:6.2f}%) | "
    ...
    keep_mask = (
        invalid_count <= max_invalid_interferograms
    )
    ...
    print(
        f"[phase] 完成：保留="
        f"{int(np.count_nonzero(keep_mask)):,}/{n_candidates:,}",
        flush=True,
    )
