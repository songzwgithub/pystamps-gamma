...
def calculate_bperp_matrix(
    ...
    *,
    ...
    if not base_paths:
        raise GammaInputError("没有输入基线文件")
    ...
    if output_file is None:
        ...
        matrix = np.memmap(
            ...
            dtype=np.float32,
            mode="w+",
            shape=(n_candidates, n_interferograms),
            order="C",
        )
    ...
    print(
        f"[bperp] 候选点={n_candidates:,}, "
        f"IFG={n_interferograms}, workers={workers}",
        flush=True,
    )

    started = time.monotonic()

    def compute_one(ifg_index: int) -> tuple[int, np.ndarray]:
        baseline_model = read_baseline_model(
            base_paths[ifg_index]
        )

        column = calculate_bperp_column(
            candidate_geometry,
            radar_geometry,
            baseline_model,
        )

        return ifg_index, column

    with ThreadPoolExecutor(
        max_workers=workers,
        thread_name_prefix="stage1-bperp",
    ) as executor:
        futures = [
            executor.submit(compute_one, ifg_index)
            for ifg_index in range(n_interferograms)
        ]

        completed = 0

        for future in futures:
            ifg_index, column = future.result()

            matrix[:, ifg_index] = column
            completed += 1

            if (
                completed == 1
                or completed % 50 == 0
                or completed == n_interferograms
            ):
                elapsed = time.monotonic() - started
                rate = completed / elapsed if elapsed > 0 else 0.0
                eta = (
                    (n_interferograms - completed) / rate
                    if rate > 0
                    else float("nan")
                )

                print(
                    f"[bperp] {completed}/{n_interferograms} "
                    f"({completed/n_interferograms*100:6.2f}%) | "
