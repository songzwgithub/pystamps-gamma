...
def parse_rslc_tab(
    ...
    *,
    ...
    for line_number, raw_line in enumerate(
        ...
        rslc = _resolve_list_path(
            fields[0],
            path,
        )

        par = _resolve_list_path(
            fields[1],
            path,
        )

        # Date is recoverable from the original path even when
        # RSLC_tab still references an old RSLC directory.
        date = extract_date(
            fields[0]
        )

        # ------------------------------------------------------
        # RSLC -> RSLC_cropped relocation support
        # ------------------------------------------------------
        #
        # A copied/cropped GAMMA project often retains the
        # original paths inside RSLC_tab.  If the listed file no
        # longer exists, safely relocate it by basename/date
        # inside the resolved RSLC directory.
        # ------------------------------------------------------

        if (
            not rslc.is_file()
            and rslc_dir is not None
        ):
            ...
            # Common canonical basename fallback.
            rslc_candidates.extend(
                [
                    rslc_dir
                    / f"{date}.rslc",
        ...
        # Parameter file relocation follows the selected RSLC.
        if (
            not par.is_file()
            and rslc_dir is not None
        ):
            ...
            if len(existing_par) == 1:
                par = existing_par[
                    0
                ]

            elif len(existing_par) > 1:

                # Prefer exact <selected-rslc>.par when present.
                exact = Path(
                    f"{rslc}.par"
                ).resolve()

                if exact in existing_par:
                    par = exact
                else:
                    raise GammaInputError(
                        f"{path}:{line_number} "
...
def load_gamma_sbas_project(
    ...
    *,
    ...
    # ==========================================================
    # Unified GAMMA path discovery
    # ==========================================================
    #
    # Defaults:
    #   RSLC        -> RSLC / RSLC_cropped
    #   DIFF        -> DIFF / DIFF_dir
    #   MLI         -> MLI_dir / MLI
    #   DEM         -> DEM_prep / DEM
    #
    # Non-default legacy function arguments remain explicit
    # overrides for backward compatibility.
    # ==========================================================

    path_cfg = ProjectPathsConfig(
        rslc_tab=(
            rslc_tab_name
            if rslc_tab_name != "RSLC_tab"
            else os.environ.get(
                "PYSTAMPS_RSLC_TAB"
            )
        ),
    ...
    try:
        resolved_paths = discover_gamma_inputs(
            root,
            config=path_cfg,
            strict=True,
        )
    ...
    rslc_tab = resolved_paths.rslc_tab
    itab = resolved_paths.itab
    diff_dir = resolved_paths.diff_dir
    mli_dir = resolved_paths.mli_dir
    dem_dir = resolved_paths.dem_dir

    assert rslc_tab is not None
    assert itab is not None
    assert diff_dir is not None
    assert mli_dir is not None
    assert dem_dir is not None

    print(
        "[GAMMA_PATHS] "
        f"RSLC={resolved_paths.rslc_dir}, "
        f"DIFF={diff_dir}, "
        f"MLI={mli_dir}, "
        f"DEM={dem_dir}",
        flush=True,
    ...
    rslc_records = parse_rslc_tab(
        rslc_tab,
        rslc_dir=resolved_paths.rslc_dir,
    )
